    ...
    public static void a() {
        ...
        while (i6 < a.length) {
            ...
            if (Integer.parseInt(split2[0]) != 0) {
                ...
                a[i6].OnUse = new ActionsSet(split2[20]);
                if (a[i6].type == Item.ItemType.POTION || a[i6].type == Item.ItemType.SCROLL) {
                    a[i6].OnUse.actions.add(new ScriptedAction(ScriptedAction.ActionType.LoseItem, Integer.toString(a[i6].item_ID)));
                }
                if (!split2[21].equals("")) {
